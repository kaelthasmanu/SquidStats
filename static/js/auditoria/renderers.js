...
(function(){
  ...
  function renderNestedAccordionResults(title, results) {
    if (!results || results.length === 0) {
      return `<div class="bg-white p-6 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">${title}</h2><p class="text-gray-500">${i18n.noRecordsFound || 'No se encontraron registros.'}</p></div>`;
    }
    const usersData = {};
    ...
    for (const username in usersData) {
      ...
      for (const date of sortedDates) {
        ...
        html += `<div class="border-b last:border-b-0"><div class="bg-blue-50 p-2 pl-6 flex justify-between items-center cursor-pointer hover:bg-blue-100" onclick="toggleDetails(this)"><h4 class="font-semibold text-blue-800"><i class="fas fa-calendar-alt mr-2"></i>${date}</h4><div class="flex items-center space-x-4 text-sm"><span class="font-medium text-blue-700"><i class="fas fa-hashtag mr-1"></i>${dateData.daily_requests.toLocaleString()} ${i18n.requestsCol || 'Peticiones'}</span><span class="font-medium text-green-700"><i class="fas fa-database mr-1"></i>${formatBytes(dateData.daily_data)}</span><i class="fas fa-chevron-down toggle-icon transition-transform text-xs"></i></div></div><div class="date-details p-3" style="display: none;"><table class="w-full text-left text-sm mt-2"><thead class="border-b-2 border-gray-300"><tr><th class="py-1 px-2 text-center">${i18n.lastAccessHeader || 'Hora (Últ. Acceso)'}</th><th class="py-1 px-2">${i18n.urlHeader || 'URL'}</th><th class="py-1 px-2 text-center">${i18n.attemptsHeader || 'Intentos'}</th><th class="py-1 px-2">${i18n.dataHeader || 'Datos'}</th></tr></thead><tbody class="divide-y divide-gray-100">`;
  ...
  function renderUserSummary(data) {
    ...
    return `<div class="bg-white p-6 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">${i18n.activitySummaryTitle || 'Resumen de Actividad'}</h2><div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6"><div class="bg-blue-100 p-4 rounded-lg text-center"><div class="text-3xl font-bold">${data.total_requests.toLocaleString()}</div><div class="text-sm">${i18n.totalRequestsStat || 'Peticiones Totales'}</div></div><div class="bg-green-100 p-4 rounded-lg text-center"><div class="text-3xl font-bold">${data.total_data_gb}</div><div class="text-sm">${i18n.gbConsumedStat || 'GB Consumidos'}</div></div><div class="bg-gray-100 p-4 rounded-lg"><h4 class="font-semibold text-sm mb-1 text-center">${i18n.responseSummaryStat || 'Resumen de Respuestas'}</h4><ul class="text-xs space-y-1">${responseSummaryHtml}</ul></div></div><div class="grid grid-cols-1 md:grid-cols-2 gap-6"><div><h3 class="text-xl font-semibold mb-2">${i18n.top15DomainsTitle || 'Top 15 Dominios Visitados'}</h3><canvas id="domain-chart"></canvas></div><div><h3 class="text-xl font-semibold mb-2">${i18n.httpResponseDistTitle || 'Distribución de Respuestas HTTP'}</h3><canvas id="response-chart"></canvas></div></div></div>`;
  }
  ...
  function renderTopUsers(data){
    ...
    return `<div class="bg-white p-6 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">${i18n.top10UsersByDataTitle || 'Top 10 Usuarios por Consumo de Datos'}</h2><table class="w-full text-left"><thead class="bg-gray-100"><tr><th class="p-3">#</th><th class="p-3">${i18n.userCol || 'Usuario'}</th><th class="p-3">${i18n.dataConsumedCol || 'Datos Consumidos'}</th></tr></thead><tbody class="divide-y">${rows}</tbody></table></div>`;
  }
  ...
  function renderTopUrls(data){
    ...
    return `<div class="bg-white p-6 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">${i18n.top15UrlsByDataTitle || 'Top 15 URLs por Consumo de Datos'}</h2><table class="w-full text-left"><thead class="bg-gray-100"><tr><th class="p-3">#</th><th class="p-3">${i18n.urlHeader || 'URL'}</th><th class="p-3">${i18n.dataConsumedCol || 'Datos Consumidos'}</th></tr></thead><tbody class="divide-y">${rows}</tbody></table></div>`;
  }
  ...
  function renderTopUsersRequests(data){
    ...
    return `<div class="bg-white p-6 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">${i18n.top10UsersByRequestsTitle || 'Top 10 Usuarios por Peticiones'}</h2><table class="w-full text-left"><thead class="bg-gray-100"><tr><th class="p-3">#</th><th class="p-3">${i18n.userCol || 'Usuario'}</th><th class="p-3">${i18n.requestsCol || 'Peticiones'}</th></tr></thead><tbody class="divide-y">${rows}</tbody></table></div>`;
  }
  ...
  function renderTopIpsData(data){
    ...
    return `<div class="bg-white p-6 rounded-lg shadow-lg"><h2 class="text-2xl font-bold mb-4">${i18n.top10IpsByDataTitle || 'Top 10 IPs por Consumo de Datos'}</h2><table class="w-full text-left"><thead class="bg-gray-100"><tr><th class="p-3">#</th><th class="p-3">${i18n.ipCol || 'IP'}</th><th class="p-3">${i18n.dataConsumedCol || 'Datos Consumidos'}</th></tr></thead><tbody class="divide-y">${rows}</tbody></table></div>`;
  }
  ...
  function renderDailyActivityChart(canvas, hourlyData){
    ...
    new Chart(canvas, {
      ...
      data:{
        labels,
        datasets:[{
          label: i18n.requestsPerHourLabel || 'Peticiones por Hora',
          data:hourlyData,
      ...
      options:{
        responsive:true,
        scales:{
          x:{ grid:{display:false}, title:{display:true,text: i18n.timeOfDayAxis || 'Hora del Día'} },
          y:{ beginAtZero:true, title:{display:true,text: i18n.numberOfRequestsAxis || 'Número de Peticiones'} }
        },
  ...
  function renderTotalDataConsumed(data, formData) {
    ...
    const title = (i18n.totalDataConsumedTitle || 'Total de Datos Consumidos del {startDate} al {endDate}')
    ...
    return `<div class="bg-white p-6 rounded-lg shadow-lg">
      ...
      <div class="text-center py-10">
        ...
        <div class="text-xl text-gray-600 mb-4">
          ${i18n.totalDataPeriod || 'Total de datos consumidos en el período seleccionado'}
        </div>
        <div class="grid grid-cols-2 gap-4 text-sm text-gray-500">
          <div>
            <strong>${totalRequests.toLocaleString()}</strong> ${i18n.totalRequestsFooter || 'peticiones totales'}
          </div>
          <div>
            <strong>${tablesProcessed}</strong> ${i18n.tablesProcessedLabel || 'tablas procesadas'}
          </div>
        </div>
        <div class="mt-4 text-xs text-gray-400">
          ${(i18n.equivalentBytes || 'Equivalente a {bytes} bytes').replace('{bytes}', totalDataBytes.toLocaleString())}
        </div>
      </div>
    </div>`;
  }
