...
def validate_paths():
    ...
    if not os.path.exists(BACKUP_DIR):
        ...
        errors.append(
            f"Sin permisos de escritura en directorio de backups: {BACKUP_DIR}"
        )
    ...
    if not os.path.exists(ACL_FILES_DIR):
        ...
        errors.append(
            f"Sin permisos de escritura en directorio de ACLs: {ACL_FILES_DIR}"
        )
...
class SquidConfigManager:
    ...
    def _validate_environment(self):
        ...
        if self.errors:
            ...
            self.is_valid = False
            logger.error(
                "SquidConfigManager no se puede inicializar debido a errores de configuración"
            )
        else:
            self.is_valid = True
    ...
    def save_config(self, content):
        ...
        try:
            ...
            if not backup_created:
                logger.warning(
                    "No se pudo crear backup, pero continuando con guardado..."
                )
    ...
    def create_backup(self):
        ...
        try:
            ...
            if not os.access(BACKUP_DIR, os.W_OK):
                logger.error(
                    f"Sin permisos de escritura en directorio de backup: {BACKUP_DIR}"
                )
                return False
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                try:
                    ...
                    logger.warning(
                        f"Error procesando delay pool en línea {line_num}: {e}"
                    )
                    continue

            logger.debug(
                f"Se encontraron {len(delay_pools)} configuraciones de delay pools"
            )
            return delay_pools
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                try:
                    ...
                    logger.warning(
                        f"Error procesando regla HTTP en línea {line_num}: {e}"
                    )
                    continue
