import os
import logging
from sqlalchemy import (create_engine, Column, Integer, String, BigInteger, Text, DateTime, inspect)                       
...
class LogMetadata(Base):
    ...
    # CORRECCIÓN: Se elimina la columna 'updated_at' del modelo para que coincida con la base de datos.
...
def create_dynamic_tables(engine):
    ...
    if not table_exists(engine, user_table_name) or not table_exists(engine, log_table_name):
        logger.info(f"Creando tablas dinámicas para hoy: {user_table_name}, {log_table_name}")
...
def get_dynamic_models(date_suffix: str):
    ...
    DynamicUser.__table__ = DynamicUser.__table__.tometadata(DynamicBase.metadata, name=user_table_name)
