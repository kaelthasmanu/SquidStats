...
def check_configuration():
    ...
    print(f"TELEGRAM_API_ID: {'✓ Configurado' if Config.TELEGRAM_API_ID else '✗ No configurado'}")
    print(f"TELEGRAM_API_HASH: {'✓ Configurado' if Config.TELEGRAM_API_HASH else '✗ No configurado'}")
    print(f"TELEGRAM_BOT_TOKEN: {'✓ Configurado' if Config.TELEGRAM_BOT_TOKEN else '✗ No configurado'}")
    print(f"TELEGRAM_PHONE: {'✓ Configurado' if Config.TELEGRAM_PHONE else '✗ No configurado'}")
    print(f"TELEGRAM_SESSION_NAME: {Config.TELEGRAM_SESSION_NAME}")
    print(f"TELEGRAM_RECIPIENTS: {Config.TELEGRAM_RECIPIENTS if Config.TELEGRAM_RECIPIENTS else '✗ No configurado'}")
    ...
    if not Config.TELEGRAM_RECIPIENTS or not any(r.strip() for r in Config.TELEGRAM_RECIPIENTS):
...
def test_proxy_parsing():
    ...
    test_cases = [
        ...
        ("http://user:pass@proxy.example.com:8080", ["proxy_type", "addr", "port", "username", "password"]),
        ...
        try:
            ...
            if expected_keys is None:
                ...
                if result and all(key in result for key in expected_keys):
                    ...
                    print(f"❌ Test {i}: '{proxy_url}' → {result} (faltan keys: {expected_keys})")
                    all_passed = False
