...
def test_docker_container_exists_returns_true_when_container_present():
    with (
        patch(
            "services.system.system_service._get_bin", return_value="/usr/bin/docker"
        ),
        patch("services.system.system_service.subprocess.run") as mock_run,
    ):
        mock_run.return_value = CompletedProcess(
            args=["/usr/bin/docker", "ps"],
            returncode=0,
            stdout=f"{DOCKER_CONTAINER_NAME}\n",
        )

        assert (
            _docker_container_exists("/usr/bin/docker", DOCKER_CONTAINER_NAME) is True
        )
...
def test_restart_squid_falls_back_to_docker_restart_when_systemctl_fails():
    ...
    def fake_run(args, check, capture_output, text, timeout):
        ...
        if args[:3] == ["/usr/bin/docker", "ps", "--filter"]:
            return CompletedProcess(
                args=args, returncode=0, stdout=f"{DOCKER_CONTAINER_NAME}\n"
            )
    ...
    with (
        patch("services.system.system_service._get_bin", side_effect=fake_get_bin),
        patch(
...
def test_reload_squid_uses_docker_reconfigure_when_systemctl_not_available():
    ...
    with (
        patch("services.system.system_service._get_bin", side_effect=fake_get_bin),
        patch("services.system.system_service.subprocess.run", side_effect=fake_run),
    ):
