...
def test_reset_uses_conntrack_for_linux_clients():
    with (
        patch("services.squid.connection_reset_service.platform.system", return_value="Linux"),
        patch("services.squid.connection_reset_service.shutil.which", return_value="/usr/sbin/conntrack"),
    ...
    assert details is None
    assert mock_run.call_args.args[0] == ["/usr/sbin/conntrack", "-D", "-s", "192.0.2.10"]


def test_reset_reports_unsupported_platform():
    with patch("services.squid.connection_reset_service.platform.system", return_value="Windows"):
        success, message, details = reset_client_connections("192.0.2.10")
