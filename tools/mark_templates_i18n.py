...
TRANSLATABLE_ATTRS = {
    ...
    "data-confirm", "data-confirm-title", "data-confirm-text",
}

# Strings to skip (technical, not user-facing)
SKIP_PATTERNS = {
    "true", "false", "none", "null", "utf-8", "UTF-8",
    "POST", "GET", "PUT", "DELETE", "PATCH",
    "text/html", "application/json", "multipart/form-data",
    "hidden", "submit", "button", "text", "password", "email", "number", "checkbox",
...
def is_translatable_text(text):
    ...
    if re.match(r'^(https?://|www\.|/|#|\.|fas |fab |far |fal |fad |fa-|bi-|text-|bg-|btn-|col-|row-|d-|p-|m-|w-|h-|flex-|grid-|items-|justify-)', stripped):
        return False
    # Skip if it looks like a Jinja variable or filter
    if re.match(r'^[a-z_][a-z0-9_]*(\.[a-z_][a-z0-9_]*)*$', stripped):
        return False
    # Skip JS code snippets
    if any(kw in stripped for kw in ["function(", "var ", "const ", "let ", "=>", "return ", "console.", "document.", "window.", "addEventListener"]):
        return False
    return True
...
def process_text_between_tags(content):
    ...
    while pos < len(content):
        ...
        tag_match = re.search(r'(<[^>]+>|{{.*?}}|{%.*?%}|{#.*?#})', content[pos:], re.DOTALL)
        ...
        if script_match:
            ...
            if close_pos >= 0:
                ...
                result.append(content[pos + tag_match.start():close_pos + len(close_pattern)])
