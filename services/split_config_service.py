...
def split_config(strict=False):
    ...
    if not os.path.exists(splitter.input_file):
        return {"status": "error", "message": f"Archivo squid.conf no encontrado en: {splitter.input_file}"}, 404
    ...
    try:
        ...
        return {
            ...
            "data": {"output_dir": splitter.output_dir, "files": results, "total_files": len(results)},
        }, 200
        ...
        return {"status": "error", "message": "No se tienen permisos suficientes para crear los archivos"}, 403
    except RuntimeError as e:
        logger.error("Error de validación: %s", e)
        return {"status": "error", "message": "Error de validación de la configuración"}, 400
    except Exception as e:
        logger.exception("Error al dividir el archivo de configuración")
        return {"status": "error", "message": "Error interno al dividir la configuración", "details": str(e)}, 500
