...
def check_squid_service():
    ...
    try:
        ...
        if result.returncode != 0:
            add_notification('error', 'Squid service is not running', 'fa-exclamation-triangle', 'squid')
        else:
            ...
            if 'error' in log_check.stdout.lower() or 'failed' in log_check.stdout.lower():
                add_notification('warning', 'Errors detected in Squid logs', 'fa-exclamation-triangle', 'squid')
                
    except subprocess.TimeoutExpired:
        add_notification('warning', 'Timeout checking Squid status', 'fa-clock', 'squid')
    ...
    try:
        ...
        if os.path.exists(log_file):
            ...
            if file_size_mb > 100:  # More than 100MB
                add_notification('warning', f'Squid log very large: {file_size_mb:.1f}MB', 'fa-file-alt', 'squid')
            
            # Check last modification (not updated in more than 5 minutes)
            last_modified = datetime.fromtimestamp(stat_info.st_mtime)
            time_diff = datetime.now() - last_modified
            if time_diff.total_seconds() > 300:  # 5 minutes
                add_notification('warning', 'Squid log not updated for more than 5 minutes', 'fa-clock', 'squid')
                
        else:
            add_notification('error', f'Squid log file not found: {log_file}', 'fa-file-exclamation', 'squid')
...
def check_system_health():
    ...
    try:
        ...
        if free_disk < 1:  # Less than 1GB free
            add_notification('error', f'Critical disk space: {free_disk:.1f}GB free', 'fa-hdd', 'system')
        elif free_disk < 5:  # Less than 5GB free
            add_notification('warning', f'Low disk space: {free_disk:.1f}GB free', 'fa-hdd', 'system')
...
def notify_squid_config_error(error_message: str):
    ...
    add_notification('error', f'Squid configuration error: {error_message}', 'fa-cog', 'squid')
