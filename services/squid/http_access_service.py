...
_KERBEROS_CHALLENGE_RE = re.compile(r"^http_access\s+deny\s+!\S+\s*$", re.I)


def _is_local_cache_manager_allow(parts: list[str]) -> bool:
    """Recognise either conventional order of the local manager exception.

    Squid evaluates ACLs in an access rule as a conjunction, so both
    ``allow manager localhost`` and ``allow localhost manager`` are the
    conventional narrow exception.  Do not accept extra/negated ACLs here:
    generic edits must not gain a way to place a client allow before the
    Kerberos challenge.
    """
...
def _is_managed_kerberos_access_line(lines: list[str], line_index: int) -> bool:
    """Return whether an HTTP rule belongs to the Kerberos managed block."""
    in_kerberos_block = False
    for index, line in enumerate(lines):
        marker = line.strip()
        if marker == _KERBEROS_ACCESS_START:
            in_kerberos_block = True
        elif marker == _KERBEROS_ACCESS_END:
            in_kerberos_block = False
        if index == line_index:
            return in_kerberos_block
    return False


def _editable_http_line(
    lines: list[str], http_lines_indices: list[int], rule_index: int
) -> tuple[int | None, str | None]:
    """Resolve a UI index while preserving the dedicated Kerberos rule."""
    if rule_index < 0 or rule_index >= len(http_lines_indices):
        return None, "Índice de regla inválido"
    line_index = http_lines_indices[rule_index]
    if _is_managed_kerberos_access_line(lines, line_index):
        return (
            None,
...
def _kerberos_policy_error(lines: list[str]) -> str | None:
    """Reject an edit that would put a normal allow before Kerberos.

    The dedicated screen owns the challenge line, but changing a neighboring
    rule through the generic editor can be equally dangerous.  A narrow local
    Cache Manager exception is the only positive allow permitted beforehand.
    """
    starts = [
        index
        for index, line in enumerate(lines)
        if line.strip() == _KERBEROS_ACCESS_START
    ]
    ends = [
        index
        for index, line in enumerate(lines)
    ...
    if len(starts) != 1 or len(ends) != 1 or ends[0] <= starts[0]:
        return (
            "El bloque Kerberos administrado está incompleto; revísalo desde "
            "Kerberos / SPNEGO antes de modificar reglas HTTP Access."
        )

    challenge_indices = [
        index
        for index in range(starts[0] + 1, ends[0])
        if _KERBEROS_CHALLENGE_RE.fullmatch(
            lines[index].strip().split("#", 1)[0].strip()
        )
    ]
    if len(challenge_indices) != 1:
        return (
            "No se pudo verificar el desafío Kerberos administrado; modifica "
            "la política desde Kerberos / SPNEGO."
        )

    for line in lines[: challenge_indices[0]]:
        parts = line.strip().split("#", 1)[0].strip().split()
        lowered = [part.casefold() for part in parts]
        if lowered[:2] != ["http_access", "allow"]:
            continue
        if _is_local_cache_manager_allow(parts):
            continue
        return (
            "La regla dejaría un allow antes del desafío Kerberos. Modifica la "
            "política desde Kerberos / SPNEGO o conserva solo la excepción local del manager."
        )
    return None
    ...
    try:
        if config_manager.is_modular:
            ...
            if http_content is not None:
                ...
                line_index, error = _editable_http_line(
                    lines, http_lines_indices, rule_index
                )
                if error:
                    return False, error
                # remove the line
        ...
        http_lines_indices = _find_http_lines(lines)
        line_index, error = _editable_http_line(lines, http_lines_indices, rule_index)
        if error:
            return False, error
        del lines[line_index]
        new_content = "\n".join(lines)
        if config_manager.save_config(new_content):
            return True, "Regla HTTP Access eliminada exitosamente"
        return False, "Error al eliminar la regla HTTP Access"
    except Exception:
    ...
    try:
        ...
        if config_manager.is_modular:
            ...
            if http_content is not None:
                lines = http_content.split("\n")
                http_lines_indices = _find_http_lines(lines)
                i, error = _editable_http_line(lines, http_lines_indices, rule_index)
                if error:
                    return False, error
                if description:
                    if i > 0 and lines[i - 1].strip().startswith("#"):
                        ...
                        i += 1
                lines[i] = new_rule
                if error := _kerberos_policy_error(lines):
                    return False, error
                new_content = "\n".join(lines)
                if config_manager.save_modular_config(
                    "120_http_access.conf", new_content
        ...
        if config_manager.save_config(new_content):
            return True, "Regla HTTP Access actualizada exitosamente"
        return False, "Error al guardar la regla HTTP Access"
    except Exception:
...
def add_http_deny_blocklist(acl_name: str, config_manager) -> tuple[bool, str]:
    ...
    try:
        ...
        return False, "Error al guardar la regla deny"
    except Exception:
...
def remove_http_deny_blocklist(acl_name: str, config_manager) -> tuple[bool, str]:
    ...
    try:
        ...
        if not config_manager.save_config(new_content):
            return False, "Error guardando config principal de http_access"
    ...
    try:
        if config_manager.is_modular:
            ...
            if http_content is not None:
                lines = http_content.split("\n")
                http_lines_indices = _find_http_lines(lines)
                current, error = _editable_http_line(
                    lines, http_lines_indices, rule_index
                )
                if error:
                    return False, error
                if direction == "up" and rule_index > 0:
                    ...
                    if _is_managed_kerberos_access_line(lines, target):
                        return (
                            False,
                            "No se puede mover una regla a través del bloque Kerberos administrado.",
                        )
        ...
        lines = config_manager.config_content.split("\n")
        http_lines_indices = _find_http_lines(lines)
        current, error = _editable_http_line(lines, http_lines_indices, rule_index)
        if error:
            return False, error
        if direction == "up" and rule_index > 0:
            target = http_lines_indices[rule_index - 1]
