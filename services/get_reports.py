...
def get_important_metrics(db: Session, UserModel, LogModel):
    ...
    try:
        ...
        top_users_by_activity = (db.query(
            UserModel.username,
            func.count(LogModel.id).label('total_visits')
        )
        ...
        response_distribution = (db.query(
            LogModel.response,
            func.count(LogModel.id).label('count')
        )
        ...
        total_stats = {
            ...
            'total_data_transmitted': db.query(func.sum(LogModel.data_transmitted)).scalar() or 0,
...
def get_metrics_by_date_range(start_date: str, end_date: str, db: Session):
    ...
    while current_dt <= end_dt:
        ...
        try:
            ...
            if not has_table(db, UserModel.__tablename__) or not has_table(db, LogModel.__tablename__):
            ...
            if 'total_stats' in daily_metrics:
                ...
                consolidated_results['total_stats']['total_users'] += stats.get('total_users', 0)
                consolidated_results['total_stats']['total_log_entries'] += stats.get('total_log_entries', 0)
                consolidated_results['total_stats']['total_data_transmitted'] += stats.get('total_data_transmitted', 0)
                consolidated_results['total_stats']['total_requests'] += stats.get('total_requests', 0)
