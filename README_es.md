<h1 align="center">
  <img alt="SQUIDSTAT logo" src="https://github.com/kaelthasmanu/SquidStats/blob/main/assets/squidstats.webp" width="300px"/><br/>
...
**Pasos de Configuración Esenciales(Versiones mas altas que la 7 no se necesita la ACL solo la parte del Manager Access):**
...
```bash
 curl -fsSL https://github.com/kaelthasmanu/SquidStats/releases/download/2.4/install.sh | sudo bash
```
o
```bash
  wget -qO- https://github.com/kaelthasmanu/SquidStats/releases/download/2.4/install.sh | sudo bash
```
...
### Instalación Manual(De la forma dificil)
...
1. Edita el archivo `.env` usando tu editor preferido (vi, vim o nano). Aquí un ejemplo con nano:

   ```bash
   nano .env
   ```

   Agrega o modifica la siguiente línea en el archivo `.env`:
...
2. Reinicia el servicio SquidStats:
...
```bash
 wget https://github.com/kaelthasmanu/SquidStats/releases/download/2.4/install.sh
```
...
Por favor asegúrate de que tu instalación de Squid coincida con esta versión o una más nueva para un rendimiento óptimo. Para entornos de contenedores Docker, se recomienda usar Alpine Linux 3.23.3 por su tamaño reducido y compatibilidad.
